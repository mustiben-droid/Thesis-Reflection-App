...
# --- 4. ממשק המשתמש ---
setup_design()
st.title("🎓 יומן תצפית מלא - מחקר תזה")
        ...
        if st.form_submit_button("💾 שמור תצפית ועדכן דרייב"):
            ...
            if svc and uploaded_files:
                for f in uploaded_files:
                    img_links.append(upload_image_to_drive(f, svc))
...
with tab2:
    ...
    st.write("כאן ניתן לאחד את כל התצפיות שנעשו עד כה לקובץ אקסל אחד בדרייב.")
...
with tab3:
    st.header("💬 צ'אט עם Gemini")
    if "messages" not in st.session_state: st.session_state.messages = []
    for message in st.session_state.messages:
        with st.chat_message(message["role"]): st.markdown(message["content"])

    if prompt := st.chat_input("שאל אותי על המחקר שלך..."):
        ...
        if os.path.exists(DATA_FILE):
            ...
            context = "להלן נתוני התצפיות:\n" + "\n".join([str(e) for e in entries[-15:]])

        with st.chat_message("assistant"):
            try:
                client = genai.Client(api_key=st.secrets["GOOGLE_API_KEY"])
                full_prompt = f"אתה עוזר מחקר אקדמי. בסיס את תשובתך על הנתונים הבאים:\n{context}\nשאלה: {prompt}"
