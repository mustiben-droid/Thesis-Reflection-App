...
st.set_page_config(page_title="מערכת תצפית אקדמית - Master 16.2", layout="wide")
...
# --- 2. פונקציות Google Drive (גרסת החיפוש האגרסיבי) ---
...
def fetch_history_from_drive(student_name, svc):
    try:
        # חיפוש רחב בכל המרחבים של הדרייב
        ...
        # ניקוי שמות למניעת שגיאות חיפוש
...
st.title("🎓 מערכת תצפית תזה - Master 16.2")
...
with tab1:
        ...
        with st.container(border=True):
            ...
            if student_name and svc:
                ...
                if drive_history:
                    ...
                    st.warning(f"🔍 לא נמצאה היסטוריה עבור {student_name}.")
    ...
    with col_chat:
        ...
        if u_input:
            ...
            השתמש במאמרים אמיתיים בלבד (2014-2026) בפורמט APA 7. אל תמציא מקורות.
            """
...
with tab3:
    ...
    if st.button("✨ בצע ניתוח עומק מכל נתוני המאסטר"):
        if svc:
            ...
            if res:
                ...
                client = genai.Client(api_key=st.secrets["GOOGLE_API_KEY"])
                prompt = f"נתח מגמות אקדמיות (2014-2026) בפורמט APA על בסיס כל המדדים: {data_summary}"
