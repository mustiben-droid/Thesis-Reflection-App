...
with tab3:
    st.header("🤖 ניתוח מגמות ומדדים סטטיסטיים")
    st.write("המערכת תחשב ממוצעים כמותיים ותייצר ניתוח AI מעמיק שייחמר בדרייב.")
    
    if st.button("✨ ייצר ניתוח עומק וטבלה סטטיסטית", use_container_width=True):
        if svc:
            with st.spinner("מחשב סטטיסטיקה ומנתח נתונים..."):
                df, _ = load_master_from_drive_internal(svc)
                if df is not None:
                    # 1. חישוב טבלת ממוצעים כמותית
                    # הגדרת העמודות לחישוב
                    score_cols = ['cat_convert_rep', 'cat_dims_props', 'cat_proj_trans', 'cat_3d_support', 'cat_self_efficacy', 'duration_min', 'drawings_count']
                    
                    # ניקוי נתונים - הפיכה למספרים
                    for col in score_cols:
                        df[col] = pd.to_numeric(df[col], errors='coerce')
                    
                    # חישוב ממוצעים לפי שיטת עבודה
                    stats_df = df.groupby('work_method')[score_cols].mean().round(2)
                    
                    # 2. הצגת המדדים ב-Streamlit
                    st.subheader("📊 ממוצעים לפי שיטת עבודה")
                    st.dataframe(stats_df, use_container_width=True)
                    
                    # 3. יצירת ה-Prompt ל-AI
                    summary_context = df.to_string()
                    stats_context = stats_df.to_string()
                    
                    client = genai.Client(api_key=st.secrets["GOOGLE_API_KEY"])
                    prompt = f"""
                    אתה עוזר מחקר אקדמי בכיר. נתח את הנתונים הבאים עבור תזת מחקר בחינוך טכנולוגי.
                    
                    הנתונים הגולמיים:
                    {summary_context}
                    
                    הממוצעים הסטטיסטיים שחושבו:
                    {stats_context}
                    
                    דרישות הניתוח:
                    1. השווה בין הממוצעים: האם יש הבדל מובהק בביצועים בין עבודה עם מודל ללא מודל?
                    2. נתח את הקשר בין זמן העבודה (duration_min) לבין איכות התוצרים.
                    3. זהה סטודנטים ספציפיים שהמודל עזר להם במיוחד לעומת כאלו שנותרו בקשיים.
                    4. ספק המלצות פדגוגיות להמשך המחקר.
                    
                    כתוב בצורה אקדמית, ללא רפרנסים מומצאים.
                    """
                    
                    response = client.models.generate_content(model="gemini-2.0-flash", contents=prompt)
                    
                    # 4. הכנת התוכן המלא לשמירה (טקסט AI + טבלה)
                    full_content_to_save = f"""
ניתוח מחקר תזה - תאריך: {datetime.now().strftime("%d/%m/%Y %H:%M")}
------------------------------------------------------------

{response.text}

------------------------------------------------------------
נספח א': טבלת ממוצעים סטטיסטית (מחושבת)
{stats_context}
                    """
                    
                    # הצגה על המסך
                    st.markdown("### 📄 הניתוח האקדמי:")
                    st.write(response.text)
                    
                    # שמירה לדרייב
                    saved_name = save_summary_to_drive(full_content_to_save, svc)
                    if saved_name:
                        st.success(f"✅ הניתוח והטבלה הסטטיסטית נשמרו בדרייב: **{saved_name}**")
                else:
                    st.error("לא הצלחתי לטעון את נתוני המאסטר.")
