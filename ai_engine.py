"""
ai_engine.py — מנוע ה-AI והסוכן החכם (גרסה 2.5)
תיקונים: מניעת סלט משתני קושי, פתרון הזרקה כפולה (Token Bloat), ושיחה משורשרת
"""
...
# העתקת המשתנים לשימוש פנימי במנוע
...
def render_ai_agent_tab():
    ...
    df_pp     = st.session_state.get("df_pp")
    ...
    if prompt:
        ...
        with st.chat_message("assistant"):
            ...
            # זיהוי תלמיד בפרומפט עם פתרון fuzzy מובנה
            payload_str = ""
            if df_master is not None:
                ...
                if found_student and found_student != st.session_state.current_analyzed_student:
                    ...
                    if df_pp is not None:
                        # שימוש בשם העמודה הקיים כפי שרוסס ב-load_prepost
                        name_col = "name_key" if "name_key" in df_pp.columns else "name"
                        df_pp[name_col] = df_pp[name_col].apply(clean_name)
                        pp_row = df_pp[df_pp[name_col] == key]
                        if not pp_row.empty:
